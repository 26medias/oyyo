				...
				scope.$watchCollection('page.components', function(a, b) {
					...
					if (!domObject.get(0)) {
						...
						for (i=0;i<scope.page.components.length;i++) {
							...
							code += '<div pagevamp-component="'+scope.page.components[i].id+'" '+scope.page.components[i].id+'>Loading '+scope.page.components[i].name+'... Please wait...</div>'
						}
						...
						for (i=0;i<scope.page.components.length;i++) {
							...
							code += template.replace('[component]', '<div pagevamp-component="'+scope.page.components[i].id+'" '+scope.page.components[i].id+'>Loading '+scope.page.components[i].name+'... Please wait...</div>');
						}
						element.html($compile(code)(scope));
					}
				});
				...
				var createHover = function() {
					if (!overlay) {
						...
						overlay.click(function() {
							overlay.fadeOut();
							element.parent().html($compile('<div '+$(element).attr('pagevamp-component')+'editor></div>')(scope));
						});
					}
