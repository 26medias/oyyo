		...
		angularApp.directive('componentLoader', function($compile, $injector) {
			var component = function(scope, element, attrs) {
				...
				scope.$watchCollection('page.components', function(a, b) {
					...
					if (!domObject.get(0)) {
						...
						for (i=0;i<scope.page.components.length;i++) {
							code += '<div '+scope.page.components[i].id+'>'+scope.page.components[i].id+' goes here</div>'
						}
						...
						var i;
						for (i=0;i<scope.page.components.length;i++) {
							code += template.replace('[component]', '<div '+scope.page.components[i].id+'>'+scope.page.components[i].id+' goes here</div>');
						}
						element.html($compile(code)(scope));
					}
				});
				...
				scope.$watchCollection('page.components', function(a, b) {
					...
					var i;
					for (i=0;i<scope.page.components.length;i++) {
						//@TODO: find a way to pass a template.
						code += '<div '+scope.page.components[i].id+'>'+scope.page.components[i].id+' goes here</div>'
					}
					element.html($compile(code)(scope));
				});
