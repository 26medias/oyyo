	...
	pagevampComponentManager.prototype.init = function() {
		...
		for (id in this.registeredId) {
			(function(id) {
				...
				console.log(">>>>",this.registeredId[id].data.editor && meanEngine.get('site').data.edit, this.registeredId[id].data.editor,meanEngine.get('site').data.edit);
				// If we're in edit mode, we'll need to also load the editor directives
				if (this.registeredId[id].data.editor && meanEngine.get('site').data.edit) {
