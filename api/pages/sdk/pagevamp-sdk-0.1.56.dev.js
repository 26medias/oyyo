...
(function(meanEngine) {
	...
	pagevampComponentManager.prototype.getTemplateFor = function(id) {
		...
		return component.data.cdnPath+component.data.template;
	}
...
(function(meanEngine) {
	...
	window.Arbiter.subscribe('pvsdk.components.loaded', function(components) {
		console.info("Everything loaded. Let's load AngularJS...");
		...
		$scope.test = function() {
			...
			$scope.safeApply(function() {
				$scope.components = [{
					...
					id:	'pva2de95dbc08f28bd185f7004d7339d4a'
				}];
			});
		}
	});
...
*/
(function(meanEngine) {
	meanEngine.get('app').directive('componentLoader', function($compile, $injector) {
		var component = function(scope, element, attrs) {
			
			scope.$watchCollection('components', function(a, b) {
				console.warn("Changes!",a);
				var code = '';
				var i;
				for (i=0;i<scope.components.length;i++) {
					code += '<div '+scope.components[i].id+' style="border:1px solid #333333;padding:5px;margin:5px;">'+scope.components[i].id+' goes here</div>'
				}
				
				console.log("code",code);
				console.log("scope",scope);
				console.log("$compile(code)(scope)",$compile(code)(scope));
